...
export default function SchedulesPage() {
  ...
  const filteredSchedules = schedules.filter((schedule) => {
    ...
    const matchesShift =
      selectedShift === "" || schedule.shift === selectedShift;
  ...
  const uniqueGradeLevels = Array.from(
    new Set(schedules.map((s) => s.gradeLevel)),
  ).sort();
  const uniqueSections = Array.from(
    new Set(schedules.map((s) => s.section)),
  ).sort();
  const uniqueShifts = Array.from(new Set(schedules.map((s) => s.shift)))
              ...
              <table className="data-table">
                ...
                <tbody>
                  {filteredSchedules.map((schedule) => (
                    <tr
                      key={schedule._id}
                      className="hover:bg-[var(--secondary)] transition-colors duration-200"
                    >
