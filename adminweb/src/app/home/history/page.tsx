...
function StudentDetailsModal({
  ...
  const filteredRecords = student.recentAttendance.filter((record) => {
    ...
    const matchesDate =
      recordSelectedDate === "" ||
      format(new Date(record.checkInTime || record.scanTime), "yyyy-MM-dd") ===
        recordSelectedDate;

    const matchesSubject =
      recordSelectedSubject === "" ||
      (record.subject || "").toLowerCase() ===
        recordSelectedSubject.toLowerCase();

    const matchesStatus =
      recordSelectedStatus === "" ||
      (record.status || "").toLowerCase() ===
        recordSelectedStatus.toLowerCase();
      ...
      <div className="relative z-10 bg-[var(--surface)] rounded-[var(--radius)] shadow-xl border border-[var(--border)] p-6 w-full max-w-7xl mx-4 max-h-[calc(100vh-5rem)] overflow-y-auto">
            ...
            <button
              ...
              onClick={() =>
                onExportPDF(
                  ...
                  `attendance_${student.name.replace(/\s+/g, "_")}_${format(new Date(), "yyyy-MM-dd")}.pdf`,
                )
              }
        ...
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="group bg-gradient-to-br from-green-50 to-green-100 p-4 rounded-xl border border-green-200/50 shadow-sm hover:shadow-md hover:from-green-100 hover:to-green-200 hover:border-green-300/50 transition-all duration-300">
            ...
            <div className="text-2xl font-bold text-green-800 group-hover:scale-105 transform transition-transform duration-300">
          ...
          <div className="group bg-gradient-to-br from-red-50 to-red-100 p-4 rounded-xl border border-red-200/50 shadow-sm hover:shadow-md hover:from-red-100 hover:to-red-200 hover:border-red-300/50 transition-all duration-300">
            ...
            <div className="text-2xl font-bold text-red-800 group-hover:scale-105 transform transition-transform duration-300">
          ...
          <div className="group bg-gradient-to-br from-yellow-50 to-yellow-100 p-4 rounded-xl border border-yellow-200/50 shadow-sm hover:shadow-md hover:from-yellow-100 hover:to-yellow-200 hover:border-yellow-300/50 transition-all duration-300">
            ...
            <div className="text-2xl font-bold text-yellow-800 group-hover:scale-105 transform transition-transform duration-300">
              {student.stats.late}
            </div>
          </div>
          <div className="group bg-gradient-to-br from-orange-50 to-orange-100 p-4 rounded-xl border border-orange-200/50 shadow-sm hover:shadow-md hover:from-orange-100 hover:to-orange-200 hover:border-orange-300/50 transition-all duration-300">
            ...
            <div className="text-2xl font-bold text-orange-800 group-hover:scale-105 transform transition-transform duration-300">
              {student.stats.cutting}
            </div>
          </div>
        </div>
            ...
            <div className="flex-1 min-w-xs">
              ...
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                  <svg
                    className="h-5 w-5 text-[var(--muted-foreground)] group-focus-within:text-[var(--primary)] transition-colors duration-300"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 20 20"
                    fill="currentColor"
                  >
              ...
              <tbody className="bg-[var(--surface)] divide-y divide-[var(--border)]">
                {filteredRecords.length === 0 ? (
                  ...
                  filteredRecords.map((record) => (
                    <tr
                      key={record._id}
                      className="hover:bg-[var(--muted)] group transition-all duration-200"
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className="text-sm text-[var(--foreground)] transition-colors duration-200">
                          {format(
                            new Date(record.checkInTime || record.scanTime),
                            "MMM dd, yyyy",
                          )}
                        </span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ring-1 ${
                            record.statusHistory &&
                            record.statusHistory.length > 0
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span
                          className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ring-1 group-hover:shadow-sm ${
                            record.status === "Present"
                              ? "bg-green-50 text-green-700 ring-green-200/50 group-hover:bg-green-100"
                              : record.status === "Late"
                                ? "bg-yellow-50 text-yellow-700 ring-yellow-200/50 group-hover:bg-yellow-100"
                                : record.status === "Absent"
                                  ? "bg-red-50 text-red-700 ring-red-200/50 group-hover:bg-red-100"
                                  : record.status === "Out"
                                    ? "bg-purple-50 text-purple-700 ring-purple-200/50 group-hover:bg-purple-100"
...
export default function HistoryPage() {
  ...
  useEffect(() => {
    const calculateStats = () => {
      const newStats = {
        ...
        cutting: attendanceRecords.filter((r) => r.status === "Cutting").length,
  ...
  const fetchStudentDetails = async (studentId: string) => {
    try {
      const response = await historyService.getStudentHistory(studentId, {
        limit: 10,
      });
        ...
        <div className="dashboard-header-inner">
          ...
          <div className="dashboard-header-refresh-box">
            <div className="flex items-center gap-2">
              ...
              {isSilentRefresh && (
                <span className="text-white/90 text-xs animate-pulse ml-2">
                  🔄 Live
                </span>
              )}
              ...
              {isLoading ? (
                ...
                attendanceRecords.map((record) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap">
                      ...
                      {record.subject?.toLowerCase() !== "general" &&
                        ...
                        (record.gradeLevel.trim() !== "" ||
                          ...
                          <span className="text-[var(--muted-foreground)] block text-xs mt-0.5">
                            {[record.gradeLevel?.trim(), record.section?.trim()]
                              .filter(Boolean)
                              .join(" - ")}
                          </span>
                        )}
                    ...
                    <td className="whitespace-nowrap">
                      <span
                        className={`px-3 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ring-1 ${
                          record.status === "Present"
                            ? "bg-green-50 text-green-700 ring-green-200/50"
                            : record.status === "Late"
