...
export const studentService = {
  async addStudent(studentData: Partial<Student>) {
    try {
      ...
      if (
        ...
        isNaN(Date.parse(transformedData.birthDate))
      ) {
      ...
      if (
        ...
        !/^\+?[\d\s-]{10,}$/.test(transformedData.phoneNumber)
      ) {
        throw new Error("Invalid phone number format");
      }

      console.log(
        "Service: Sending request to backend with data:",
        transformedData,
      );
  ...
  async getAllStudents(options?: {
    ...
    try {
      ...
      const url = params.toString()
        ? `${API_BASE_URL}/api/students?${params}`
      ...
      // Always return [] — never throw. A failure here must not crash
      // the navbar notification count or any other dependent caller.
      console.warn(
        "getAllStudents failed, returning []:",
  ...
  async generateQRCode(id: string): Promise<{ qrCode: any; student: Student }> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/students/${id}/qr-code`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async getQRCode(
    id: string,
  ): Promise<{ qrCode: any; studentId: string; fullName: string }> {
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/students/${id}/qr-code`,
      );
  ...
  async regenerateAllQRCodes(): Promise<{
    message: string;
    successCount: number;
    errorCount: number;
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/students/qr-codes/regenerate-all`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async bulkUpdateStudents(
    ...
    try {
      ...
      throw error instanceof Error
        ? error
        : new Error("Failed to bulk update students");
    }
  },
};
