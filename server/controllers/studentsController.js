...
const createStudent = async (req, res) => {
  try {
    ...
    if (pendingRequests.has(requestKey)) {
      ...
      return res
        .status(409)
        .json({ error: "Request already being processed. Please wait..." });
    }
    ...
    console.log(
      "Student ID char codes:",
      studentId?.split("").map((c) => c.charCodeAt(0)),
    );
...
const getAllStudents = async (req, res) => {
  try {
    ...
    const students = await Student.find(filter)
      // Exclude large base64 fields from list responses.
      // photo (profile pic) + qrCode.image (QR png) = ~1.4 MB for 57 students.
      // These are only needed in the individual student detail view.
      .select("-photo -qrCode.image")
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(Number(limit));
...
const getStudentsByTeacherSchedule = async (req, res) => {
  try {
    ...
    if (!teacherSchedules || teacherSchedules.length === 0) {
      console.log(
        `No schedules found for teacher: ${teacherName} in ${gradeLevel} ${section}`,
      );
      return res.status(200).json([]);
    }

    console.log(
      `Found ${teacherSchedules.length} schedule(s) for teacher: ${teacherName} in ${gradeLevel} ${section}`,
    );
    console.log(
      "Teacher schedules:",
    ...
    console.log(
      `Found ${students.length} students in ${gradeLevel} ${section} matching teacher's shift(s)`,
    );
...
const getQRCode = async (req, res) => {
  try {
    ...
    try {
      student = await Student.findById(id).select(
        "qrCode studentId fullName parentInfo",
      );
      console.log("Search by MongoDB _id - Found:", !!student);
    } catch (err) {
      // If not a valid MongoDB ObjectId, search by studentId instead
      console.log(
        "MongoDB _id search failed (expected for non-ObjectId):",
        err.message,
      );
      student = null;
    }
    ...
    if (!student) {
      ...
      student = await Student.findOne({ studentId: id }).select(
        "qrCode studentId fullName parentInfo",
      );
  ...
  try {
    ...
    for (const student of students) {
      try {
        ...
        console.error(
          `Error regenerating QR code for student ${student.studentId}:`,
          error,
        );
        errorCount++;
      }
...
const clearDatabase = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({
        message: `Cleared ${result.deletedCount} students from database`,
      });
  } catch (error) {
...
const validateCheckIn = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to validate check-in" });
  }
};
...
const validateCheckOut = async (req, res) => {
  try {
    ...
    console.log(
      `  - Status: ${hasOpenCheckOut ? "BLOCKED (no check-in but already checked out)" : hasNoCheckIn ? "BLOCKED (no check-in)" : "ALLOWED (has open check-in)"}`,
    );
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to validate check-out" });
  }
};
...
const bulkUpdateStudents = async (req, res) => {
  try {
    ...
    res
      .status(400)
      .json({ error: error.message || "Failed to bulk update students" });
  }
};
