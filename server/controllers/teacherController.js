...
const createTeacher = async (req, res) => {
  ...
  try {
    ...
    let subjectsArray = Array.isArray(subjects)
      ? subjects
      : (subject ? [subject] : []).filter(Boolean);
    ...
    if (!username || !name || !email || !phoneNumber || !password) {
      return res.status(400).json({
        error:
          "Missing required fields: username, name, email, phoneNumber, password are required",
      });
    }
    ...
    const createdTeacher = await Teacher.findOne({ username }).select(
      "-password",
    );
...
const getAllTeachers = async (req, res) => {
  try {
    ...
    console.log("getAllTeachers called with params:", {
      page,
      limit,
      search,
      subject,
    ...
    // Get teachers with pagination (excluding password and large base64 fields)
